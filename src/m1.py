...
class CircleChanger(object):
    ...
    def get_distance_from(self, point):
        ...
        temp = self.circle.center
        return ((temp.x - point.x) ** 2 + (temp.y - point.y) ** 2) ** (1/2)
    ...
    def swallow(self, other_circle_changer):
        ...
        c1 = self.circle.center
        c2 = other_circle_changer.circle.center

        cnew = rg.Point((math.fabs(c2.x + c1.x)/2), (math.fabs(c2.y + c1.y)/2))
        distance = self.get_distance_from(c2)

        locors = self.colors + other_circle_changer.colors

        return CircleChanger(cnew.x, cnew.y, distance / 2, 'red', locors)

    def change_color(self, index_of_color):
        """
        What comes in:
          -- self
          -- A nonnegative integer that is less than the length
    ...
    def change_to_original_color(self):
        ...
        self.circle.fill_color = self.ocolor
    ...
    def change_to_next_color_in_tuple(self):
        ...
        self.tuplecounter += 1
        if self.tuplecounter == len(self.colors):
            self.tuplecounter = 0

        self.circle.fill_color = self.colors[self.tuplecounter]
